...
_rag_instance = None
def set_rag_instance(rag): global _rag_instance; _rag_instance = rag
def get_rag_instance(): return _rag_instance
...
def calculate(expression: str):
    try:
        return eval(expression.replace("^", "**"), {"__builtins__": None},
            {"sin": math.sin, "cos": math.cos, "sqrt": math.sqrt, "pi": math.pi, "e": math.e})
    except: return "Error"
...
def read_file(file_path: str) -> str:
    if ".." in file_path: return "Denied"
    try:
        with open(f"ai_agent/{file_path}", "r", encoding="utf-8") as f: return f.read()[:5000]
    except: return "Error"
...
def get_all_tools():
    return [get_current_time, calculate, search_web, query_knowledge_base, read_file, get_weather, github_search]